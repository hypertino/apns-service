...
class ApnsClientImpl(configuration: ApnsClientConfiguration) extends StrictLogging with ApnsClient {
      ...
      override def operationComplete(future: util.concurrent.Future[PushNotificationResponse[T]]): Unit = {
        if (future.isSuccess){
          ...
          if(pushNotificationResponse.isAccepted) {
            logger.trace("APNS accepted notification: " + pushNotificationResponse)
          }else{
